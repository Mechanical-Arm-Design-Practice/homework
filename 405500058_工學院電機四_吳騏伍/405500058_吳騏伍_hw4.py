...
zipcode = {\
    ...
    "新北市": {"萬里區": 207, "金山區": 208, "板橋區": 220, "汐止區": 221, "深坑區": 222, "石碇區": 223, "瑞芳區": 224, "平溪區": 226, \
        ...
        "樹林區": 238, "鶯歌區": 239, "三重區": 241, "新莊區": 242, "泰山區": 243, "林口區": 244, "蘆洲區": 247, "五股區": 248, "八里區": 249, \
        "淡水區": 251, "三芝區": 252, "石門區": 253}\
        }

def list_zip(city):
    print(zipcode[city].values())


def area_to_zip(area):
    for city,all_area in zipcode.items():
        for target_area in all_area :
            if(target_area == area):
                print(city,all_area[target_area])


def zip_to_area(zip):
    for city,all_area in zipcode.items():
        for target_area in all_area :
            if(str(all_area[target_area]) == zip):
                print(target_area)


input_city=input('輸入城市名稱：')
list_zip(input_city)
input_area=input('輸入區域名稱：')
area_to_zip(input_area)
input_zip=input('輸入郵遞區號：')
