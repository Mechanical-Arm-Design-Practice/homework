...
def find_phrase(text,find_str):
    ...
    while(beg < text_length):
        remain = text[beg:end]
        if(remain.find(find_str)!= -1):
            position = beg + remain.find(find_str)
            print(position)
            beg = position + len(find_str)
            counter+=1
        else:
            break
    print("總共有",counter,"個",find_str, "\n")
